...
public:
    ...
    char operator[](int index) const {
        if (index >= 0 && index < length) {
            ...
            return '\0'; // Возвращаю нулевой символ для некорректного индекса
        }
    }
};
...
public:
    ...
    BitString(const char* s) : String(s) {
        // Проверка на другие символы кроме 0 и 1
        for (int i = 0; i < GetLength(); ++i) {
            if ((*this)[i] != '0' && (*this)[i] != '1') {
                // Если есть символ не равняется 0 или 1 просто чистим строку
                Clear();
                break;
            }
        }
